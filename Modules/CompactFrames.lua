...
if not VanillaEnhancedOptions["CompactFramesUnitWidth"] then
	VanillaEnhancedOptions["CompactFramesUnitWidth"] = 76
end
if not VanillaEnhancedOptions["CompactFramesUnitHeight"] then
	VanillaEnhancedOptions["CompactFramesUnitHeight"] = 38
end
...
local function ResizeMemberFrame(frameName, width, height)
	...
	local hpHeight = height - 8
	if hpHeight < 1 then hpHeight = 1 end

	local healthBar = getglobal(frameName .. "HealthBar")
	if healthBar then
		healthBar:SetWidth(width)
		healthBar:SetHeight(hpHeight)
	end

	local healthBarPrediction = getglobal(frameName .. "HealthBarPrediction")
	if healthBarPrediction then
		healthBarPrediction:SetWidth(width)
		healthBarPrediction:SetHeight(hpHeight)
	end

	local powerBar = getglobal(frameName .. "PowerBar")
	if powerBar then
		powerBar:SetWidth(width)
		powerBar:SetHeight(8)
		powerBar:SetPoint("TOPLEFT", frame, "TOPLEFT", 0, -hpHeight)
	end

	-- Background frame is the first child.
	...
	-- Reposition Power Textures if they exist
	local powerTexture1 = getglobal(frameName .. "PowerTexture1")
	if powerTexture1 then
		powerTexture1:SetWidth(width)
		powerTexture1:SetPoint("CENTER", frame, "CENTER", 0, -(hpHeight / 2) + 2)
	end
	local powerTexture2 = getglobal(frameName .. "PowerTexture2")
	if powerTexture2 then
		powerTexture2:SetWidth(width)
		powerTexture2:SetPoint("CENTER", frame, "CENTER", 0, -(hpHeight / 2) - 6)
	end
end
...
local function UpdateMemberFrame(unitInfo, frameName)
	...
	ResizeMemberFrame(frameName, VanillaEnhancedOptions["CompactFramesUnitWidth"], VanillaEnhancedOptions["CompactFramesUnitHeight"])
...
local function ResizeGroupFrame(groupIdx, width, height)
	local frame = getglobal("GroupFrame" .. groupIdx)
	if not frame then return end
...
local function UpdatePartyGroup()
	...
	local width = VanillaEnhancedOptions["CompactFramesUnitWidth"] or 76
	local height = VanillaEnhancedOptions["CompactFramesUnitHeight"] or 38
	ResizeGroupFrame(1, width, height)
...
local function UpdateRaidGroups()
	...
	for i = 1, 8 do
		ResizeGroupFrame(i, width, height)
	end
